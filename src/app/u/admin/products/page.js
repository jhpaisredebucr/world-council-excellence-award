...
export default function AdminProductsPage() {
  ...
  async function uploadProductImage(file) {
    ...
    uploadData.append("public_id", `product-${Date.now()}`);
    ...
    if (!uploadRes.ok || !uploadJson.success) {
      throw new Error(uploadJson.message || "Product image upload failed");
    }
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const imagePublicId = await uploadProductImage(imageFile);
      ...
      setProducts((prev) => [createJson.product, ...prev]);
      setForm(initialForm);
      setImageFile(null);
  ...
  async function handleDeleteProduct(product) {
    ...
    try {
      ...
      if (!deleteRes.ok || !deleteJson.success) {
        throw new Error(deleteJson.message || "Failed to remove product");
      }
  ...
  return (
    <div className="space-y-6">
      <ConfirmModal
        ...
        content={
          productToDelete
            ? `Are you sure you want to remove "${productToDelete.product_name}"?`
            : ""
        }
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 md:p-6">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Add new products and upload product images for the shop.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 md:p-6">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-xl font-semibold text-gray-800">Current Products</h2>
          <button
            ...
            {products.map((product) => (
              ...
              >
                ...
                <div className="space-y-1 p-3">
                  ...
                  >
                    {deletingId === product.id ? "Removing..." : "Remove Product"}
                  </button>
                </div>
              </div>
            ))}
